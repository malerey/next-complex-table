    ...
    <tr>
      <td colSpan={colSpan} className="border border-border p-0">
        <div className="bg-muted p-4">
          <h4 className="font-medium text-sm mb-3 text-foreground">Tasks by Category:</h4>
            ...
            {Object.entries(tasksByCategory).map(([category, tasks]) => (
              <div key={category}>
                ...
                <div className="space-y-2 ml-2">
                  {tasks.map((task) => (
                    <div key={task.id} className="flex items-center gap-3 text-sm p-2 bg-card rounded border border-border shadow-sm">
                      <span
                        className={`text-sm ${
                      ...
                      <div className="flex-1">
                        <span className={task.status === "completed" ? "line-through text-muted-foreground" : "text-card-foreground"}>
                          {task.title}
                        </span>
                      </div>
                      ...
                      <div className="text-xs text-muted-foreground min-w-[80px]">{task.assignee || "Unassigned"}</div>
                      ...
                      <span className="text-xs text-muted-foreground/60 min-w-[30px] text-right">({task.weight})</span>
                    </div>
                  ))}
                </div>
              </div>
